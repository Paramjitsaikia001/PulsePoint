...
const SetLocation = () => {
    ...
    const handleSetCurrentLocation = async () => {
        ...
        try {
            // Fixed Promise syntax for JavaScript
            ...
            >
                <div className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <p className="text-gray-600">Use your current location or search for a specific place</p>
                    </div>
                    ...
                    <motion.button
                        ...
                        disabled={isLoading}
                        className="w-full px-4 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
                    >
                        {isLoading ? (
                                    ...
                                    <ul className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                                        {suggestions.map((suggestion, index) => (
                                            <motion.li
                                                ...
                                                className={`px-4 py-2 text-sm cursor-pointer flex items-start gap-2 hover:bg-gray-50 transition-colors ${index !== suggestions.length - 1 ? "border-b border-gray-100" : ""
                                                    }`}
            ...
            <AnimatePresence>
                {toastMessage && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 50 }}
                        className={`fixed bottom-4 right-4 p-4 rounded-lg shadow-lg max-w-xs ${toastMessage.variant === "destructive" ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"
                            }`}
                    >
